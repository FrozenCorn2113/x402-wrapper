...
def roster_report() -> dict:
    ...
    return {
        ...
        "how_to_verify": (
            ...
            "9. Liveness threshold: a roster counts as EVIDENCE of "
            "independent watchers only with at least 3 'holding' records, "
            "each with last_pull_age_seconds under 7 days (604800s). Below "
            "that bar the design is design-capable only — the machinery "
            "works, but the witness set is too thin to trust. Rationale: "
            "1 holder could be the operator, 2 is coincidence, 3 is the "
            "smallest fabrication-needs-coordination quorum; 7-day "
            "staleness because evidence needs a plausibly-still-watching "
            "witness. This number is POLICY, not derived — independence "
            "is the load-bearing property, and it is established "
            "off-server (point 3, evidence_url), never by this server's "
            "word. As of 2026-09-25 the roster holds 1 announced / "
            "0 holding records — under the bar, design-capable only, "
            "stated plainly."
        ),
        "honesty": (
