...
- **Deploy saga:** dashboard deploy of 7a9f389 succeeded (dep-daq3eivlot8c73f6cgl0, 36.8s) but verification caught a real bug — /skill.md returned "skill.md not installed" because the Dockerfile's explicit COPY list omitted skill.md. Fixed (COPY skill.md), committed 81916e3. A steered redeploy task FAILED at hand-off without triggering a deploy (live site still on 7a9f389: no skill.md content, no X-Payment-Required header, no top-level 402 price). Fresh deploy task spawned for 81916e3 — result pending at run-notes time.
