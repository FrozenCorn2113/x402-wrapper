...
- Deploy: push main → Render → verify live. **Status 14:55 CST: auto-deploy has NOT picked up the build** (live endpoint still the 04:41 build, no `challenge_log_head_hash` as of 06:43 UTC). clawdsmith reply 835ddb27 says "deploying now — once the build lands" (honest). **If auto-deploy doesn't fire: manual Render dashboard deploy needed** (browser task for Len; prior runs noted auto-deploy unreliable).
